    ...
    .then(result => {
     ...
     // console.log('promise ready')
      dataLayerSL.push('user_data_android', (userData.map((v) => v).filter(v => v)).join('-_-'));
        ...
        if(app_data.search(/android-app:\/\/ru.sberbankmobile\/|sberbankonline/)>=0) {
            if (typeof(web_link_and_option) == 'undefined' || web_link_and_option == ''){
                if ((need_web == false) || (web_link_desk.search("person/dist_services/inner_apps") >= 0 && web_link_desk.search("person/dist_services/inner_apps/packs") <= 0 )) {
                    ...
                    var dp_and_opr = dp_and.replace(new RegExp("^android-app:\/\/ru.sberbankmobile\/|^android-app:\/\/ru.sberbankmobile\/android-app\/ru.sberbankmobile\/|^sberbankonline:\/\/", "gm"), "intent://ru.sberbankmobile/") + "#Intent;scheme=android-app;package=ru.sberbankmobile;S.browser_fallback_url=" + encodeURIComponent(document.location.origin + document.location.pathname + '?round__two_and=' + web_and_data.replace(/#/, "_anchor_sl_")+ '&need_web=' + need_web + "&sl=" + sl_name + '&external_source=' + app_data.split('external_source=')[1]) + ";end";
                    setTimeout(function () { redirect_app_android('app', dp_and_opr) }, 100);
                    }
                }
        } else {
            if (option == 1) {
                ...
                setTimeout(function () { redirect_app_android('web1', web_and_data)}, 100)
            }
            else if (option == 3) {
            ...
            setTimeout(function () { redirect_app_android('app', dp_and)}, 100)
            setTimeout(function () { redirect_app_android('web1', web_and_data)}, 900)
        }
           ...
           // iOS
            ...
            setTimeout(function(){
                redirect_app_ios('_no_app', ios_apps_dp[1] != '' ? fios : web_data)
            }, 0)
            setTimeout(function(){
                redirect_app_ios('_has_app', web_data)
            }, 500)
        ...
        //             setTimeout(function () {
        //             redirect_app_ios('1_web_background_blure', web_data);
        //             }, 500);
        ...
        //         setTimeout(function () {
        //             if (noblurrun) { window.location.href = fios;};
        //         }, 1000);

        //     } else {

        //         setTimeout(function () {
        //             if (noblurrun) {redirect_app_ios('1_web_background_time', web_data);};
        //         }, 1000);
        //     }
...
if (document.location.search.search('round__two_and=') == 1){
    ...
    console.log('r2_dl_push', 'web') // need update!!!

    window.location.href = url; // need update!!!

    },200);

    } else {

    dataLayerSLR2.push('r2_android_go_to_store', 'tracker') // need update!!!

    console.log('r2_dl_push', 'store') // need update!!!
    ...
    function redirect_app_iosR2(aR2, lR2)
    {
        dataLayerSLR2.push(aR2, lR2);
       return  window.location.href=lR2
    }

    var dcs = decodeURIComponent(document.location.search).split(/SL_DLTR|SL_WLTR/);

    var noblurrunR2 = true;
    window.onblur = function () {
        noblurrunR2 = false;
        // dataLayerSLR2.push('info_ios2_web_no_app', decodeURIComponent(dcs[1]));
        // window.location.href = decodeURIComponent(dcs[2]);
        redirect_app_iosR2('info_ios2_web_no_app', decodeURIComponent(dcs[2]));

        setTimeout(function () {
            redirect_app_iosR2('info_ios2_web_background_blure', decodeURIComponent(dcs[2]));
            }, 500);

        };

        setTimeout(function () {
        // dataLayerSLR2.push('ios_2_go_web_background', dcs[2]);
             //window.location.href = decodeURIComponent(dcs[2]);
            
             if (noblurrunR2) {redirect_app_iosR2('info_ios2_web_background_time', decodeURIComponent(dcs[2]));};
         }, 1000);
        ...
        redirect_app_iosR2('info_ios2_app', decodeURIComponent(dcs[1]));
        },  200);
